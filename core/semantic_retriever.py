...
# MODEL_PATH = "/home/ali/.cache/huggingface/hub/models--allenai--specter/snapshots/81cbfb43d4fc2e728d5b4201ce14987db8d0854c"
MODEL_PATH = "./models/all-MiniLM-L6-v2"

class SemanticRetriever:
    def __init__(self):
        # self.model = SentenceTransformer(MODEL_PATH, token=token)
        self.model = SentenceTransformer(MODEL_PATH , local_files_only=True)
        self.index = None
        self.docs = []

    def build_index(self, docs):
        # encode article an put in FAISS(Facebook AI Similarity Search)
        self.docs = docs
        texts = []
        for doc in docs:
    ...
    def search(self, query, top_k=5):
        ...
        query_embedding = query_embedding.astype('float32')  # convert to float32 for FAISS
